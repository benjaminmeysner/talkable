...
public class UserServiceImpl implements UserService {
	...
	@Override
	public boolean isTheCreator(User user, Event event) {
		boolean exists = false;

		for (int i = 0; i < user.getEventsCreatedList().size(); i++) {
			if (user.getEventsCreatedList().get(i).getId().equals(event.getId())) {
				exists = true;
			}
		}
		return exists;
